...
- (void)requestStoriesList
{
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray * _Nullable objects, NSError * _Nullable error) {
        if(!error)
        {
            ...
            _storiesTableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(loadMore)];
            [self getSubmittersOfStory:_storiesArr];
        }
        else
        {
            MAXLog(@"get storiesList error:%@",error.localizedDescription);
            if (error.code==101)
            {
                [self.storiesTableView.mj_footer endRefreshingWithNoMoreData];
            }else if(error.code == -1009 ||  error.code == -1005)
            {
                MAXAlert(@"网络连接错误或超时，请检查网络后下拉刷新重试");
            }
