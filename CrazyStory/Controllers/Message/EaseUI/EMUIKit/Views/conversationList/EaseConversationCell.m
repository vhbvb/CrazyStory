...
- (void)setModel:(id<IConversationModel>)model
{
    ...
    if (self.showAvatar)
    {
        ...
        {
            [AVUser loadUserWithUserID:_model.avatarURLPath result:^(AVUser *user, NSError *error) {
                if (!error)
                {
                    
                    self.titleLabel.text = (user.username.length>12 ? [user.username substringToIndex:12] : user.username) ;
                    //                if (!_model.avatarImage)
                    //                {
