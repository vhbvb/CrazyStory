...
@interface MAXChatViewController ()<EaseMessageViewControllerDelegate,EaseMessageViewControllerDataSource,UIAlertViewDelegate,EMClientDelegate>
...
@implementation MAXChatViewController
...
- (void)_setupBarButtonItem
{
    ...
    {
        UIButton *clearButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 22, 22)];
        clearButton.accessibilityIdentifier = @"clear_message";
        [clearButton setBackgroundImage:[UIImage imageNamed:@"deleteItem"] forState:UIControlStateNormal];
        [clearButton addTarget:self action:@selector(deleteAllMessages:) forControlEvents:UIControlEventTouchUpInside];
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:clearButton];
    }
    else
    {//群聊
        UIButton *detailButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 22, 22)];
        detailButton.accessibilityIdentifier = @"detail";
        [detailButton setBackgroundImage:[UIImage imageNamed:@"groupItem"] forState:UIControlStateNormal];
...
{
    MAXLog(@"user objectID :%@",messageModel.message.from);
    
    [AVUser loadUserWithUserID:messageModel.message.from result:^(AVUser *user, NSError *error) {
        if (!error&&user)
        {
            MAXUserDetailsViewController * vc = [[MAXUserDetailsViewController alloc] initWithAVUser:user];
               ...
               selectAtTarget:(EaseSelectAtTargetCallback)selectedCallback
{
    ...
    if (chatGroup) {
        if (!chatGroup.occupants) {
            __weak MAXChatViewController* weakSelf = self;
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    __strong MAXChatViewController *strongSelf = weakSelf;
                    if (strongSelf) {
                        [strongSelf hideHud];
                        if (error) {
                            ...
                            MAXContactSelectionViewController *selectController = [[MAXContactSelectionViewController alloc] initWithContacts:members];
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    model.avatarURLPath = message.from ;
...
- (NSArray*)emotionFormessageViewController:(EaseMessageViewController *)viewController
{
    ...
    NSArray *namesArr = @[@"哭",@"开心",@"嘚瑟",@"滑稽",@"再见",@"飞升",@"吃货",@"晚安",@"飞吻",@"轻视",@"醒醒",@"生日快乐",@"固执",@"抓狂",@"逗",@"嗨"];
    int index = 0;
    for (NSString *name in names) {
        index++;
        EaseEmotion *emotion = [[EaseEmotion alloc] initWithName:namesArr[index-1] emotionId:[NSString stringWithFormat:@"em%d",(1000 + index)] emotionThumbnail:[NSString stringWithFormat:@"%@_cover",name] emotionOriginal:[NSString stringWithFormat:@"%@",name] emotionOriginalURL:@"" emotionType:EMEmotionGif];
...
- (void)backAction
{
    ...
    [[MAXEMHelper shareHelper] setChatVC:nil];
...
- (void)showGroupDetailAction
{
    ...
    if (self.conversation.type == EMConversationTypeGroupChat) {
        MAXChatGroupDetailViewController *detailController = [[MAXChatGroupDetailViewController alloc] initWithGroupId:self.conversation.conversationId];
