...
@interface MAXCallManager()<EMChatManagerDelegate, EMCallManagerDelegate, EMCallBuilderDelegate>
...
{
    static MAXCallManager *callManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        callManager = [[MAXCallManager alloc] init];
    });
...
- (void)_timeoutBeforeCallAnswered
{
    ...
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"没有响应，自动挂断" delegate:self cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
    [alertView show];
}
                        ...
                        type:(EMCallType)aType
{
    ...
    void (^completionBlock)(EMCallSession *, EMError *) = ^(EMCallSession *aCallSession, EMError *aError){
        MAXCallManager *strongSelf = weakSelf;
        if (strongSelf) {
            if (aError || aCallSession == nil) {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"创建实时通话失败，请稍后重试" message:aError.errorDescription delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
                [alertView show];
                return;
            }
